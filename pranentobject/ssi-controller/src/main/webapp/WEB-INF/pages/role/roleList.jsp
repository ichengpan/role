...
<!-- 搜索面板 -->
	...
	<div id="role_tb">
		<!-- Single button -->
		<div class="btn-group">
		  <button type="button" class="btn btn-success" onclick="show_add_roles_dialog()">新增roleList</button>
		</div>
		<div class="btn-group">
	...
	<script type="text/javascript">
			...
			$('#role_dg').bootstrapTable({
			    ...
			    columns: [{
			        ...
			        formatter:function(value, row, index) {
			        	...
			        	+ '<button type="button" class="btn btn-xs btn-success" onclick="show_edit_dialog(\'' + row.roleID + '\')">编辑</button>'
			        	+ '</div>&nbsp;&nbsp;'
			        	+ '<div class="btn-group">'
		...
		function show_add_roles_dialog(){
			BootstrapDialog.show({
				title:"角色管理>>新增角色",
				message: $('<div></div>').load('<%=request.getContextPath() %>/role/toUpdateRolePage.jhtml'),
				buttons: [{
	                label: '确定',
	                cssClass:"btn btn-success",
	                action: function(dialogItself){
	                	// 使用ajax保存结果
	                	alert($("#add_roles_form").serialize())
	                	$.ajax({
	                		type:"post",
	                		url:"<%=request.getContextPath() %>/role/insertRole.jhtml",
	                		data:$("#add_roles_form").serialize(),
	                		dataType:"json",
	                		success:function(data) {
	                			$('#role_dg').bootstrapTable("refresh");
	                			//关闭对话框
	                			dialogItself.close();
	                		},
	                	});
		...
		//修改角色
		function show_edit_dialog(role_id){
			BootstrapDialog.show({
				title:"角色管理>>编辑角色",
				message: $('<div></div>').load('<%=request.getContextPath() %>/role/toUpdateRolePage.jhtml?roleID=' + role_id),
				buttons: [{
	                label: '确定',
	                cssClass:"btn btn-success",
	                action: function(dialogItself){
